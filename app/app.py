# app_matnr_scan_with_decl_final.py
...
DECL_ENTRY = re.compile(
    ...
    r"|LIKE\s+(?P<like>\w+)"  # LIKE other
    r"|\((?P<charlen>\d+)\)\s*TYPE\s*C"  # (n) TYPE C
    r")?",
    re.IGNORECASE
)
...
def build_response(unit: Unit, issues: List[Dict[str, Any]]) -> Dict[str, Any]:
    """
    Map internal MATNR issues to final-format findings.
    issues_type is prefixed with MATNR_ as per your Option C.
    """
    return {
        "pgm_name": unit.pgm_name,
        "inc_name": unit.inc_name,
